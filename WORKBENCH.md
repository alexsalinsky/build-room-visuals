...
**Known:** `plan-the-problem` parks its callout badge on top of its own title, and the WHEN card's divider hairline overshoots the card edge. Both are in the source slide and both reproduce unskinned, so all three language variants have them. Worth fixing at the source, which fixes them everywhere at once.
...
**[DECISION] The brand was measured off humansvrobots.co, not read off the BrandKit, and both the hub and the skin were rebuilt paper-first.** The first version used black `#0B0B0B` as the ground, because the BrandKit names it first in the palette. Alex: "I'm seeing a totally black background and when I go to the website, it's not a totally black background." The guide lists the colors but not their proportions. On the live site `body` is white with ink text; black shows up as an occasional band, never as the page. Volt appears about twice a view, as a highlighter behind words and as the fill on the one card you're heading toward. So: white ground, ink type, zero border-radius anywhere, 2px borders on anything boxed, hairlines at `rgba(11,11,11,.14)`, hot `#FF5A1F` for the dot in the mark since volt never sits on white.

That inversion was also the source of every contrast bug in the first attempt. The source slides are already black on white, so branding them is a light transform: swap the two fonts in, square the corners, put volt on the destination. `recolorInk()` shrank to two small value maps and the runtime contrast sweep became unnecessary.

**[DECISION] The Spanish layer's strength follows which language is driving.** Blue is not a brand color, so all four blues collapse to ink. But a fixed tint broke ES-only mode: Spanish is the entire slide there and rendered at 55%, washed out. Now the `-es` classes default to full ink and step back to `ink55` only under `body.mode-both`, where the italic is already doing the separating. Captions and detail lines stay quiet in both languages, matching the English hierarchy. The SVG diagrams carry their Spanish as an inline `fill` that no selector can reach, so the generator points those at `var(--es)` and the skin swings the variable by mode. Same call as the 2026-08-12 `dream-to-phases-bi` entry below.

**[DECISION] The opener carries volt behind "Stay Human." / "Sigue siendo humano."** It's the site's one repeated move and the deck had nothing of it. Anchored to the preceding `<br/>` so it only fires inside the thesis block. On the bilingual slide only the leading language takes it, since the site never fills two things at once.

**Verified:** all four archetypes at 1600x950 on the paper skin. Plain card slide, SVG diagram with its volt finale box, chat mock, and the bilingual slide in both EN+ES and ES-only. No colored text left anywhere on the bilingual slide.

**Pushed:** `8bedf99` (hub + the first seven `-hvr.html` slides) and `1011b37` (the pill fix), both scoped by Alex, and both now superseded by the paper rebuild. Everything since is uncommitted, as are the pending proof-library, talks and photo changes.
